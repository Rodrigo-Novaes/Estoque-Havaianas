...
def api_alterar_permissao():
    ...
    try:
        ...
        nomes_permissoes = {
            'backup_visualizar': 'Visualizar',
            'backup_criar': 'Criar',
            'backup_baixar': 'Baixar',
            'backup_restaurar': 'Restaurar',
            'backup_excluir': 'Excluir'
        }
        
        nome_permissao = nomes_permissoes.get(permissao, permissao)
        acao = 'ativada' if valor else 'desativada'
        ...
        return jsonify({
            'success': True,
            'mensagem': f'✅ Permissão {nome_permissao} {acao} para {usuario.nome or usuario.username}!'
        })
...
# Coloque esta variável no início do arquivo, depois dos imports
IMPRESSORAS_CACHE = None

@app.route('/config')
@login_required
@admin_required
def config_index():
    global IMPRESSORAS_CACHE
    ...
    # ===== DETECÇÃO DE IMPRESSORAS (APENAS 1 VEZ) =====
    if IMPRESSORAS_CACHE is None:
        print("🔍 Detectando impressoras do Windows (primeira vez)...")
        impressoras = []
        try:
            import subprocess
            import json
            ...
            if resultado.returncode == 0 and resultado.stdout.strip():
                ...
                print(f"✅ Encontradas {len(impressoras)} impressoras")
            else:
                impressoras = [
                    {'id': 1, 'nome': 'Microsoft Print to PDF', 'tipo': 'relatorio', 'padrao': True}
                ]
        except Exception as e:
            print(f"❌ Erro: {e}")
            impressoras = [
        ...
        IMPRESSORAS_CACHE = impressoras
    else:
        print("📋 Usando cache de impressoras")
        impressoras = IMPRESSORAS_CACHE
    # =================================================
